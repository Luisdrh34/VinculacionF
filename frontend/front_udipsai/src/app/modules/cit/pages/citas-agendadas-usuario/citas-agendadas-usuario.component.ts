...
@Component({
  selector: 'app-citas-agendadas-usuario',
  templateUrl: './citas-agendadas-usuario.component.html',
  styleUrls: ['./citas-agendadas-usuario.component.scss'],
})
export class CitasAgendadasUsuarioComponent implements OnInit {
  citasRespaldo: any[] = [];
  citasFiltrados: any[] = [];
  citas: any[] = [];
  ...
  loadCitas(event?: any): void {
    let profesional = { idProfesional: 0 };

    const storedProfesional = sessionStorage.getItem('profesional');
    if (storedProfesional) {
      profesional = JSON.parse(storedProfesional);
    }

    if (this.filtroCitas !== '') {
      this.citaService.obtenerTodasLasCitasFiltro(this.filtroCitas).subscribe(
        (data: any) => {
      ...
      this.citaService
        ...
        .subscribe(
          ...
          (error) => {
            console.error(
              'Error fetching appointments by professional:',
              error
            );
          }
        );
  ...
  finalizarCita(citaId: any) {
    this.citaService.finalizarCita(citaId).subscribe({
      next: (data) => {
        this.toastr.success('Cita terminada con éxito.');
        this.loadCitas();
      },
      error: (error) => {
        this.toastr.error(
