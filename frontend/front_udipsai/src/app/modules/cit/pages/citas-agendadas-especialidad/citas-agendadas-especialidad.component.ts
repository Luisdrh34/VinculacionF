...
@Component({
  selector: 'app-citas-agendadas-especialidad',
  templateUrl: './citas-agendadas-especialidad.component.html',
  styleUrls: ['./citas-agendadas-especialidad.component.scss'],
})
export class CitasAgendadasEspecialidadComponent implements OnInit {
  citasRespaldo: any[] = [];
  citasFiltrados: any[] = [];
  citas: any[] = [];
  ...
  loadCitas(event?: any) {
    if (this.filtroCitas != '') {
     ...
     if (event) {
      ...
      let areasUsuarioLogueado = this.usuarioLogueado.usuarioAreas.map(
        (area: any) => area.area.idArea
      );

      this.citaService
        .obtenerCitasPorAreas(areasUsuarioLogueado, this.currentPage, this.rows)
        .subscribe((data) => {
          this.citas = data.content;
          this.totalRecords = data.totalElements;
        });
